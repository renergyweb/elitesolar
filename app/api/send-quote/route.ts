...
const resend = new Resend('re_NkdFkmiy_JTgfybtMC3bxHokXdS9sB5XA');

export async function POST(request: Request) {
  try {
    const data = await request.json();
    const { nombre, correo, whatsapp, tipoProyecto, gasto, zona, tipoPanel, paneles, inversion, roi } = data;
    ...
    const htmlContent = `
      <div style="font-family: Arial, sans-serif; max-w: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
        <div style="background-color: #059669; padding: 24px; text-align: center;">
          <h1 style="color: white; margin: 0; font-size: 24px;">Cotización Solar - Elite Solar</h1>
        </div>
        <div style="padding: 32px; background-color: #f8fafc;">
          ...
          <div style="background-color: white; padding: 24px; border-radius: 8px; border: 1px solid #e2e8f0; margin: 24px 0;">
            ...
            <ul style="list-style: none; padding: 0; color: #334155; line-height: 2;">
              <li><strong>Gasto Bimestral Actual:</strong> $${gasto.toLocaleString()} MXN</li>
              <li><strong>Tipo de Panel:</strong> ${tipoPanel}</li>
              <li><strong>Paneles Recomendados:</strong> ${paneles}</li>
              <li><strong>Inversión Estimada:</strong> $${inversion.toLocaleString()} MXN</li>
        ...
        <div style="background-color: #0f172a; padding: 16px; text-align: center; color: #94a3b8; font-size: 12px;">
          <p style="margin: 0;">© ${new Date().getFullYear()} Elite Solar. Todos los derechos reservados.</p>
        </div>
      </div>
    `;
    ...
    await resend.emails.send({
      from: 'Elite Solar <onboarding@resend.dev>',
      to: ['elitesolar504@gmail.com'],
    ...
    await resend.emails.send({
      from: 'Elite Solar <onboarding@resend.dev>',
      to: [correo],
      subject: 'Tu Cotización Solar - Elite Solar',
      html: htmlContent,
    });
